...
export interface OneDriveFsOptions {
  onConfigChanged?: (config: OnedriveConfig) => void | Promise<void>;
}
...
export class OneDriveFs implements WorkerFs {
  ...
  constructor(config: OnedriveConfig, private readonly options: OneDriveFsOptions = {}) {
    this.config = config;
  ...
  private async getHeaders(): Promise<Record<string, string>> {
    if (!this.refreshPromise) {
      ...
      this.refreshPromise = (async () => {
        const latest = await ensureFreshToken(previous);
        this.config = latest;
        if (latest.accessToken !== previous.accessToken
          || latest.refreshToken !== previous.refreshToken
          || latest.accessTokenExpiresAt !== previous.accessTokenExpiresAt) {
          await this.options.onConfigChanged?.(latest);
        }
      })().finally(() => {
  ...
  async put(key: string, content: ArrayBuffer | Uint8Array): Promise<void> {
    ...
    if (content.byteLength < DIRECT_UPLOAD_MAX) {
      ...
      if (!res.ok) throw new StorageRequestError(res.status, `onedrive put failed (${res.status})`);
    } else {
      ...
      if (!sessionRes.ok) throw new StorageRequestError(sessionRes.status, `onedrive createUploadSession failed (${sessionRes.status})`);
      ...
      while (start < uint8.byteLength) {
        ...
        if (!chunkRes.ok) throw new StorageRequestError(chunkRes.status, `onedrive upload chunk failed (${chunkRes.status})`);
        start = end;
      }
    }
  }
  ...
  async get(key: string): Promise<ArrayBuffer> {
    ...
    if (!metaRes.ok) throw new StorageRequestError(metaRes.status, `onedrive get metadata failed (${metaRes.status})`);
    ...
    if (!contentRes.ok) throw new StorageRequestError(contentRes.status, `onedrive download failed (${contentRes.status})`);
  ...
  async delete(key: string): Promise<void> {
    ...
    if (!res.ok && res.status !== 404) throw new StorageRequestError(res.status, `onedrive delete failed (${res.status})`);
  }
  ...
  async head(key: string): Promise<boolean> {
    ...
    throw new StorageRequestError(res.status, `onedrive head failed (${res.status})`);
  }
  ...
  async list(prefix: string): Promise<string[]> {
    ...
    while (deltaUrl) {
      ...
      if (!res.ok) throw new StorageRequestError(res.status, `onedrive list failed (${res.status})`);
