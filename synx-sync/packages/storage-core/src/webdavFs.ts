...
export class WebDAVFs implements WorkerFs {
  ...
  private async ensureBaseDir(): Promise<void> {
    ...
    for (const segment of this.remoteBaseDirSegments) {
      ...
      if (!this.isSuccessfulMkcol(res.status)) {
        throw new StorageRequestError(res.status, `webdav mkdir failed (${res.status})`);
      }
    }
  }
  ...
  async put(key: string, content: ArrayBuffer | Uint8Array): Promise<void> {
    ...
    throw new StorageRequestError(res.status, `webdav put failed (${res.status})`);
  }
  ...
  async get(key: string): Promise<ArrayBuffer> {
    ...
    if (!res.ok) throw new StorageRequestError(res.status, `webdav get failed (${res.status})`);
  ...
  async delete(key: string): Promise<void> {
    ...
    throw new StorageRequestError(res.status, `webdav delete failed (${res.status})`);
  }
  ...
  async head(key: string): Promise<boolean> {
    ...
    throw new StorageRequestError(res.status, `webdav head failed (${res.status})`);
  }
...
<D:propfind xmlns:D="DAV:">
  <D:prop>
    ...
    while (queue.length > 0) {
      ...
      if (!res.ok) throw new StorageRequestError(res.status, `webdav list failed (${res.status})`);
