...
export async function exchangeCodeForToken(params: {
  ...
  if ('error' in json) {
    throw new Error(`onedrive token exchange failed: ${(json as TokenError).error_description}`);
  }
...
export async function refreshAccessToken(params: {
  ...
  const json = await res.json() as TokenResponse | TokenError;
  if ('error' in json) {
    throw new Error(`onedrive token refresh failed: ${(json as TokenError).error_description}`);
  }
  return json as TokenResponse;
}
