...
- **仓库层零 D1 / 零 KV**：HEAD、提交、检查点、blob 全部是用户存储上的对象。D1 仅保留用户账户、存储配置、API token 与保留策略配置；KV 仅用于限流（防御机制，不属于仓库数据）。并发原子性依赖存储条件写（S3/R2/MinIO 支持 `If-Match`；WebDAV 用锁对象降级），不需要数据库事务。
