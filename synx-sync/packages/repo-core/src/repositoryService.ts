...
export class ExternalRepositoryLockRequiredError extends Error {
  constructor() {
    super('storage backend requires external repository serialization');
    this.name = 'ExternalRepositoryLockRequiredError';
  }
}
...
/**
 * 原子推进 HEAD（比较并交换）。
 * - 首次设置（expected === null）：优先 putIfNoneMatch；否则必须显式声明已有外部锁。
 * - 更新：优先「验证读 + putIfMatch(etag)」实现 CAS；无条件写后端同样要求外部锁。
...
async function casWriteHead(
  ...
  if (expected === null) {
    ...
    if (!externalLock) throw new ExternalRepositoryLockRequiredError();
  ...
  for (let attempt = 0; attempt < LOCK_ACQUIRE_RETRIES; attempt++) {
    try {
      ...
      if (acquired) {
        ...
        try {
          ...
          try {
            ...
            if (now === token) await deleteLockIgnoringNonAuth(fs, lockKey);
          } catch (error) {
      ...
      if (await isLockExpired(fs, lockKey)) {
        await deleteLockIgnoringNonAuth(fs, lockKey);
      }
      await sleep(LOCK_RETRY_DELAY_MS);
    } catch (error) {
      if (isAuthenticationError(error)) throw error;
...
async function deleteLockIgnoringNonAuth(fs: WorkerFs, lockKey: string): Promise<void> {
  try {
    await fs.delete(lockKey);
  } catch (error) {
    if (isAuthenticationError(error)) throw error;
  }
}
...
async function readCommit(fs: WorkerFs, syncFolder: string, commitId: string): Promise<RepoCommit | null> {
  ...
  const raw = await fs.get(key);
  try {
    return JSON.parse(textDecoder.decode(raw)) as RepoCommit;
  } catch {
...
async function loadCheckpoint(fs: WorkerFs, syncFolder: string, commitId: string): Promise<Map<string, RepoFile>> {
  ...
  const raw = await fs.get(key);
  try {
    const parsed = JSON.parse(textDecoder.decode(raw)) as { files: RepoFile[] };
...
export async function initRepository(input: InitRepositoryInput): Promise<{ head: RepositoryHead; commit: RepoCommit }> {
  const { storageId, syncFolder, fs, author } = input;
  ...
  const ok = await casWriteHead(fs, syncFolder, null, head, input.externalLock === true);
...
export async function finalizeCommit(input: FinalizeCommitInput): Promise<{ commit: RepoCommit; head: RepositoryHead }> {
  const { storageId, syncFolder, fs, baseCommitId, baseGeneration, author, message } = input;
  ...
  const ok = await casWriteHead(fs, syncFolder, head, nextHead, input.externalLock === true);
...
export async function restoreRepository(input: RestoreInput): Promise<{ preview?: RepoRestorePreviewData; commit?: RepoCommit; head?: RepositoryHead }> {
  const { storageId, syncFolder, fs, toCommitId, dryRun, author } = input;
  const head = await readHead(fs, syncFolder);
  if (!head) throw new RepoNotInitializedError();
...
async function readCommitFast(fs: WorkerFs, syncFolder: string, commitId: string): Promise<RepoCommit | null> {
  const raw = await fs.get(commitKey(syncFolder, commitId));
  try {
    return JSON.parse(textDecoder.decode(raw)) as RepoCommit;
  } catch {
    return null;
...
/** 批量删除对象；deleteMany 缺失时逐个删除，认证失败立即上抛，其余失败留待重试。 */
async function deleteKeys(fs: WorkerFs, keys: string[]): Promise<{ deleted: number; failed: string[] }> {
  if (keys.length === 0) return { deleted: 0, failed: [] };
  if (fs.deleteMany) {
    const result = await fs.deleteMany(keys);
    return { deleted: result.deleted, failed: result.failed > 0 ? keys : [] };
  }
  let deleted = 0;
  const failed: string[] = [];
...
function isAuthenticationError(error: unknown): error is { status: 401 | 403 } {
...
export async function gcRepository(input: GcRepositoryInput): Promise<GcRepositoryResult> {
  ...
  if (state.pending.length > 0) {
    ...
    const result = await deleteKeys(fs, batch);
    const pending = [...result.failed, ...rest];
    if (pending.length > 0) {
      state.pending = pending;
      await saveGcState(fs, syncFolder, state);
      return { scanned: 0, deleted: result.deleted, deletedCommits: state.deletedCommits, more: true };
    }
    await deleteGcState(fs, syncFolder);
    return { scanned: 0, deleted: result.deleted, deletedCommits: state.deletedCommits, more: false };
  }
  ...
  if (toDelete.length > 0) {
    ...
    if (pending.length > 0) {
      ...
      await saveGcState(fs, syncFolder, { ...state, commits: [], cursor: null, pending, deletedCommits });
